...
public class blast implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equals("blast")) {
            if (sender instanceof Player) {
                Player player = (Player) sender;
                if (player.hasPermission("snr.blast")) {
                    if (args.length == 0) {
                        for (Entity players : player.getNearbyEntities(20, 20, 20)) {
                            if (players instanceof Player) {
                                Vector vector = player.getLocation().getDirection();
                                vector = vector.multiply(1.1);
                        ...
                        if (player.hasPermission("snr.blast.others")) {
                            ...
                            if (target instanceof Player) {
                                ...
                                player.sendMessage("§3§lSN§c§lR §6§l> §r§3Blasted away everyone near §d" + target.getDisplayName() + "§3.");
